...
urlpatterns = [
    ...
    re_path(r'^password_reset/confirm/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,32})/$',
            authViews.PasswordResetConfirmView.as_view(template_name="accounts/register_set_password.html", success_url=reverse_lazy("password_reset_complete")), name='password_reset_confirm'),
